...
rankclass <-
  read_tsv("../data/metaphlan3.merged.abundance.profile.all.tsv") %>%
  select(clade_name) %>%
  filter(str_detect(clade_name, "s__[^|]*$")) %>%
  rowwise() %>%
  mutate(
    kingdom = str_split(clade_name, "\\|")[[1]][1],
    phylum = str_split(clade_name, "\\|")[[1]][2],
    class = str_split(clade_name, "\\|")[[1]][3],
    order = str_split(clade_name, "\\|")[[1]][4],
    family = str_split(clade_name, "\\|")[[1]][5],
    genus = str_split(clade_name, "\\|")[[1]][6],
    species = str_split(clade_name, "\\|")[[1]][7]
  ) %>%
  arrange(clade_name) %>%
  select(-clade_name) %>%
  as.data.frame()
rownames(rankclass) <- rankclass$species
