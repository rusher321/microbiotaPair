...
rownames(ko) <- ko[,1]
ko <- ko[,-1]
colnames(ko) <- gsub("-RPKs", "", colnames(ko))
ko2 <- ko[-c(1:2), ]
...
metadata2 <- openxlsx::read.xlsx("../data/metadata.xlsx", sheet = 3)
########## spearman correlation #####################
res1_spearman <- rmeta::myspearman()
...
source("../R/pcorPair.R")
metadataVar <- colnames(metadata2)
confounder <- c("BMI")
# # split g0 & g1
...
res2 <- PcorPair(microbiota = microdataf, metadata = metadata2, metadataVar = metadataVar, confounder = confounder,method = "s", time_varname = "Group2")
